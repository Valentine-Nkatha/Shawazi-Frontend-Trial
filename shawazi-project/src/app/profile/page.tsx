...
//       fileUploadElement.click(); // Safe to call click() since we checked for null
//     } else {
//       console.error("File upload element not found"); // Log error if the element is not found
//     }
//   }}
//   className="flex items-center mt-4 text-foreground"
// >
...
const Profile = () => {
  const [imagePreview, setImagePreview] = useState<string | null>(null); // Correct type here
  const [isEditing, setIsEditing] = useState(false);
  const [formData, setFormData] = useState({
    firstName: "",
    lastName: "",
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        localStorage.setItem("profileImage", reader.result as string); // Cast as string
      };
      reader.readAsDataURL(file);
    }
  };
