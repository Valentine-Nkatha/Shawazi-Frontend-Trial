...
const ChatRoom: React.FC = () => {
  ...
  const hardCodedMessages: { [key: string]: string[] } = {
    ...
    buyer: [
      "I'm interested in your land!",
      "Is it still on sale?",
      "What's the asking price for the property?",
      "Can we arrange a visit?",
    ],
    lawyer: [
      "Thank you for your message. How can I assist you with this property transaction?",
      "Have you reviewed the contract details yet?",
      "Let me know if you need any legal advice regarding this property.",
    ],
  };
  ...
  useEffect(() => {
    ...
    if (userRole && userName) {
      ...
      console.error("User role or name is missing in cookies");
    }
  }, []);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        const responseRole =
          currentUserRole === "lawyer" ? selectedUser.role : "lawyer";
        if (hardCodedMessages[responseRole]) {
          const responseMessage =
            hardCodedMessages[responseRole][
              Math.floor(Math.random() * hardCodedMessages[responseRole].length)
            ];
          addMessage(responseMessage, selectedUser.first_name);
        } else {
          console.error("Invalid role for generating a response message");
        }
      }, 1000);
    } catch (error) {
  ...
  if (error) {
    return (
      <div className="flex justify-center items-center h-full">
        Error loading users: {error.message || "An unknown error occurred"}
      </div>
    );
  }
      ...
      <div className="flex flex-col flex-grow w-full">
        <header className="bg-white shadow-sm p-3 flex items-center border-b border-gray-200">
          <h1 className="text-xl font-semibold text-green-700 flex-1">
            {selectedUser
              ? `Chat with ${selectedUser.first_name} (${selectedUser.role})`
        ...
        <main className="flex-1 overflow-y-auto p-4 bg-gray-50">
          {filteredMessages.map((message: MessageType) => (
            ...
            >
              <div
                className={`p-2 rounded-lg shadow-md mb-2 ${
                  message.sender === currentUserName
